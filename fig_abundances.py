...
l1 = legend(hcn, hcnl, loc="upper left",  fontsize=11)
l2 = legend(h2o, h2ol, loc="upper right", fontsize=11)
gca().add_artist(l1)
plt.text(1e-7, 5e-6, "Blecic/Cubillos", color=tc)
plt.errorbar(1.13e-4, 0.5e-5, xerr=xerr[7], fmt=".", ms=5, color="b")
plt.errorbar(1.60e-4, 0.9e-5, xerr=xerr[6], fmt=".", ms=5, color="r")

plt.text(1e-9, 5e-5, "MacDonald/Madhusudhan", color=tc)
plt.errorbar(1.5e-5, 0.5e-4, xerr=xerr[5], fmt=".", ms=5, color="b")
plt.errorbar(6.0e-5, 0.9e-4, xerr=xerr[4], fmt=".", ms=5, color="r")

plt.text(1e-7, 5e-4, "Line", color=tc)
plt.errorbar(0.11749, 0.5e-3, xerr=xerr[3],fmt=".",ms=5,color="b")
plt.errorbar(4.47e-4, 0.9e-3, xerr=xerr[2],fmt=".",ms=5,color="r")

plt.text(1e-8, 5e-3, "Waldmann", color=tc)
plt.errorbar(1.44e-5, 0.5e-2, xerr=xerr[1], fmt=".", ms=5, color="b")
plt.errorbar(9.33e-6, 0.9e-2, xerr=xerr[0], fmt=".", ms=5, color="r")

plt.ylim(np.amax(press), np.amin(press))
plt.xlim(1e-12, 1.0)
plt.xlabel("Mole mixing fraction")
plt.ylabel("Pressure  (bar)")
plt.savefig("figs/abundances_wasp63b_HCN_H2O.pdf", bbox_inches="tight")
